...
def train_autoregressor_autoencoder(config):
    ...
    train_loader = DataLoader(TensorDataset(x0_, xdiff_), batch_size=int(config["batch_size"]), shuffle=True)

    # autoregressor autoencoder architecture
    architectures = [
    ...
    class Autoencoder(nn.Module):
        def __init__(self, latent_dim, input_shape, layers):
            ...
            current_features = layers[-1]
            for units in reversed(layers[:-1]):
                decoder_layers.append(nn.Linear(current_features, units))
                decoder_layers.append(nn.ReLU())
                current_features = units
            decoder_layers.append(nn.Linear(current_features, np.prod(input_shape[-2:])))
